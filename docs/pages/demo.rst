...
.. tip:: OSX loopback addresses

    OSX does not properly adhere to RFC3330 such that you will need to manually create aliases for each IFACE + NODE__ID
    used. In our example, with IFACE being ``127.9.0.0`` and NODE_ID being ``42`` this would be
    ``ifconfig lo0 alias 127.9.0.42 up`` (see

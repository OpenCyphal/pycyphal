...
class SerialTransportStatistics(pycyphal.transport.TransportStatistics):
    in_bytes: int = 0
    in_frames: int = 0
...
class SerialTransport(pycyphal.transport.Transport):
    """
    The Cyphal/Serial transport is designed for OSI L1 byte-level serial links and tunnels,
    ...
    def __init__(
        ...
        *,
        ...
        self._input_registry: typing.Dict[pycyphal.transport.InputSessionSpecifier, SerialInputSession] = {}
        self._output_registry: typing.Dict[pycyphal.transport.OutputSessionSpecifier, SerialOutputSession] = {}
        ...
        if not serial_port.is_open:
            raise pycyphal.transport.InvalidMediaConfigurationError("The serial port instance is not open")
    ...
    def get_output_session(
        ...
        if specifier not in self._output_registry:
            ...
            if (
                isinstance(specifier.data_specifier, pycyphal.transport.ServiceDataSpecifier)
    ...
    async def spoof(self, transfer: pycyphal.transport.AlienTransfer, monotonic_deadline: float) -> bool:
        ...
        if isinstance(ss.data_specifier, pycyphal.transport.ServiceDataSpecifier) and (src is None or dst is None):
    ...
    def _handle_received_frame(self, timestamp: Timestamp, frame: SerialFrame) -> None:
        ...
        if frame.destination_node_id in (self._local_node_id, None):
            for source_node_id in {None, frame.source_node_id}:  # pylint: disable=use-sequence-for-iteration
                ss = pycyphal.transport.InputSessionSpecifier(frame.data_specifier, source_node_id)
                try:
    ...
    async def _send_transfer(
        ...
        try:  # Jeez this is getting complex
                    ...
                    if timeout > 0:
                        ...
                        try:
                            ...
                            if self._capture_handlers:  # Create a copy to decouple data from the serialization buffer!
                                ...
                                pycyphal.util.broadcast(self._capture_handlers)(cap)
            ...
            if self._closed:
                raise pycyphal.transport.ResourceClosedError(f"{self} is closed, transmission aborted.") from ex
            raise
        else:
    ...
    def _reader_thread_func(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        def callback(ts: Timestamp, buf: memoryview, frame: typing.Optional[SerialFrame]) -> None:
            ...
            if self._capture_handlers:
                pycyphal.util.broadcast(self._capture_handlers)(SerialCapture(ts, buf, own=False))
        ...
        try:
            ...
            if self._closed or not self._serial_port.is_open:
                ...
                handle_internal_error(
                    _logger,
                    ex,
                    "%s: Reader thread has failed, the instance with port %s will be terminated",
                    self,
                    self._serial_port,
                )
            self._closed = True

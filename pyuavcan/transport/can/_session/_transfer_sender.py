...
import itertools
import pyuavcan
from .._frame import UAVCANFrame, TRANSFER_CRC_LENGTH_BYTES
...
def serialize_transfer(compiled_identifier:     int,
                       ...
                       max_frame_payload_bytes: int) -> typing.Iterable[UAVCANFrame]:
    """
    ...
    if payload_length <= max_frame_payload_bytes:               # SINGLE-FRAME TRANSFER
        if payload_length > 0:
            padding_length = UAVCANFrame.get_required_padding(payload_length)
        ...
        yield UAVCANFrame(identifier=compiled_identifier,
        ...
        if last_frame_payload_length + TRANSFER_CRC_LENGTH_BYTES >= max_frame_payload_bytes:
            ...
            padding = _PADDING_PATTERN * UAVCANFrame.get_required_padding(last_frame_data_length)
...
def _unittest_can_serialize_transfer() -> None:
    ...
    def run(compiled_identifier:     int,
            transfer_id:             int,
            fragmented_payload:      typing.Sequence[memoryview],
            max_frame_payload_bytes: int) -> typing.Iterable[DataFrame]:
    ...
    assert mkf(0xbadc0fe, b'Hello', 0, True, True, True) \
        == one(run(0xbadc0fe, 32, [mv(b'Hell'), mv(b'o')], 7))

    assert mkf(0xbadc0fe, bytes(range(60)) + b'\x00\x00\x00', 19, True, True, True) \
        == one(run(0xbadc0fe, 32 + 19, [mv(bytes(range(60)))], 63))

...
from pyuavcan_v0_5.transport import CompoundValue, PrimitiveValue, ArrayValue, VoidValue
try:
...
def _to_json_compatible_object_impl(obj):
    ...
    if isinstance(obj, CompoundValue):
        output = dict()
        for field_name, field in pyuavcan_v0_5.get_fields(obj).items():
            if pyuavcan_v0_5.is_union(obj) and pyuavcan_v0_5.get_active_union_field(obj) != field_name:
                continue
    ...
    # ArrayValue
        ...
        t = pyuavcan_v0_5.get_uavcan_data_type(obj)
...
def _to_yaml_impl(obj, indent_level=0, parent=None, name=None, uavcan_type=None):
    ...
    # Decomposing PrimitiveValue to value and type. This is ugly but it's by design...
    if isinstance(obj, PrimitiveValue):
        uavcan_type = pyuavcan_v0_5.get_uavcan_data_type(obj)
        obj = obj.value
    ...
    if isinstance(obj, CompoundValue):
        ...
        if first_field and not pyuavcan_v0_5.is_union(obj):
            if indent_level > 0:
                indent_newline()
            write('{}')
...
def value_to_constant_name(struct, field_name, keep_literal=False):
    ...
    uavcan_type = pyuavcan_v0_5.get_uavcan_data_type(struct)
    if pyuavcan_v0_5.is_request(struct):
        consts = uavcan_type.request_constants
        fields = uavcan_type.request_fields
    elif pyuavcan_v0_5.is_response(struct):
...
if __name__ == '__main__':
    # to_yaml()
    print(to_yaml(pyuavcan_v0_5.protocol.NodeStatus()))

    info = pyuavcan_v0_5.protocol.GetNodeInfo.Response(name='legion')
    info.hardware_version.certificate_of_authenticity = b'\x01\x02\x03\xff'
    print(to_yaml(info))

    lights = pyuavcan_v0_5.equipment.indication.LightsCommand()
    lcmd = pyuavcan_v0_5.equipment.indication.SingleLightCommand(light_id=123)
    lcmd.color.red = 1
    lcmd.color.green = 2
    lcmd.color.blue = 3
    ...
    print(to_yaml(pyuavcan_v0_5.equipment.power.BatteryInfo()))
    print(to_yaml(pyuavcan_v0_5.protocol.param.Empty()))

    getset = pyuavcan_v0_5.protocol.param.GetSet.Response()
    print(to_yaml(getset))
    pyuavcan_v0_5.switch_union_field(getset.value, 'empty')
    print(to_yaml(getset))

    # value_to_constant_name()
    print(value_to_constant_name(
        pyuavcan_v0_5.protocol.NodeStatus(mode=pyuavcan_v0_5.protocol.NodeStatus().MODE_OPERATIONAL),
        'mode'
    ))
    print(value_to_constant_name(
        pyuavcan_v0_5.protocol.NodeStatus(mode=pyuavcan_v0_5.protocol.NodeStatus().HEALTH_OK),
        'health'
    ))
    print(value_to_constant_name(
        pyuavcan_v0_5.equipment.range_sensor.Measurement(reading_type=pyuavcan_v0_5.equipment.range_sensor.Measurement()
                                                  .READING_TYPE_TOO_FAR),
        'reading_type'
    ))
    print(value_to_constant_name(
        pyuavcan_v0_5.protocol.param.ExecuteOpcode.Request(opcode=pyuavcan_v0_5.protocol.param.ExecuteOpcode.Request().OPCODE_ERASE),
        'opcode'
    ))
    print(value_to_constant_name(
        pyuavcan_v0_5.protocol.file.Error(value=pyuavcan_v0_5.protocol.file.Error().ACCESS_DENIED),
        'value'
    ))
    print(value_to_constant_name(
        pyuavcan_v0_5.equipment.power.BatteryInfo(status_flags=
                                           pyuavcan_v0_5.equipment.power.BatteryInfo().STATUS_FLAG_NEED_SERVICE),
        'status_flags'
    ))
    print(value_to_constant_name(
        pyuavcan_v0_5.equipment.power.BatteryInfo(status_flags=
                                           pyuavcan_v0_5.equipment.power.BatteryInfo().STATUS_FLAG_NEED_SERVICE |
                                           pyuavcan_v0_5.equipment.power.BatteryInfo().STATUS_FLAG_TEMP_HOT |
                                           pyuavcan_v0_5.equipment.power.BatteryInfo().STATUS_FLAG_CHARGED),
        'status_flags'
    ))
    print(value_to_constant_name(
        pyuavcan_v0_5.protocol.AccessCommandShell.Response(flags=
                                                    pyuavcan_v0_5.protocol.AccessCommandShell.Response().FLAG_SHELL_ERROR |
                                                    pyuavcan_v0_5.protocol.AccessCommandShell.Response().
                                                    FLAG_HAS_PENDING_STDOUT),
        'flags'
    ))

    # Printing transfers
    node = pyuavcan_v0_5.make_node('vcan0', node_id=42)
    node.request(pyuavcan_v0_5.protocol.GetNodeInfo.Request(), 100, lambda e: print(to_yaml(e)))
    node.add_handler(pyuavcan_v0_5.protocol.NodeStatus, lambda e: print(to_yaml(e)))
    node.spin()
